...
class Trainer:
    def __init__(
        ...
        if any(isinstance(c, Checkpoint) for c in callbacks):
            ...
            checkpoint = Checkpoint("val/loss" if "skeletons" not in logger.dir else "val/acc")
    ...
    def train_step(
        ...
        loss, outputs = model(batch, self.global_step) # self.current_epoch) # ORIGINALLY self.global_step
    ...
    def test_loop(
        ...
        if validation:
            ...
            #     print("\n\nmodel.max_grad =", model.max_grad.item())
        else:
            print_str = "Testing"
            prefix = "test"
            if hasattr(self.checkpoint, 'model_max_grad'):
                print("\n\nmodel.max_grad =", self.checkpoint.model_max_grad)
        ...
        if validation:
            for callback in self.callbacks:
                callback.on_test_end(self, model, optimizer, metrics, nums)
            ...
            for batch in train_loader:
                ...
                if self.global_step % self.val_check_interval == 0:
                    if val_loader is not None and self.limit_val_batches > 0:
                        with torch.no_grad():
                            self.test_loop(
                                model, optimizer, val_loader, validation=True, nums=len(train_loader.dataset),
                            )
